...
# Chemistry Arrays
periodictable = ["Bq", "H", "He", "Li", "Be", "B", "C", "N", "O", "F", "Ne", "Na", "Mg", "Al", "Si", "P", "S", "Cl",
...
sterimol_atomtypes = ["C", "C2", "C3", "C4", "C5/N5", "C6/N6", "C7", "C8", "H", "N", "C66", "N4", "O", "O2", "P", "S",
                      "S1", "F", "C1", "S4", "B1", "I", 'Si']

# CPK VdW radii in pm
cpk_radii = [150, 160, 160, 150, 170, 170, 170, 150, 100, 150, 170, 145, 135, 135, 140, 170, 100, 135, 180, 140, 195,
             215, 210]
...
def bondiRadius(massno):
    ...
    bondi = [0.0, 1.09, 1.40, 1.82, 2.00, 2.00, 1.70, 1.55, 1.52, 1.47, 1.54, 2.27, 1.73, 2.00, 2.10, 1.80, 1.80, 1.75,
             1.88, 2.75, 2.00, 2.00, 2.00, 2.00, 2.00, 2.00, 2.00, 2.00, 1.63, 1.40, 1.39, 1.87, 2.00, 1.85, 1.90,
             1.85, 2.02, 2.00, 2.00, 2.00, 2.00, 2.00, 2.00, 2.00, 2.00, 2.00, 1.63, 1.72, 1.58, 1.93, 2.17, 2.00, 2.06,
             1.98, 2.16, 2.00, 2.00, 2.00, 2.00, 2.00, 2.00, 2.00, 2.00, 2.00, 1.72, 1.66, 1.55, 1.96, 2.02, 2.00, 2.00,
             2.00,
...
def dihedral(atoma, atomb, atomc, atomd):
    ...
    torsion = 180.0 / math.pi * math.acos((ax * bx + ay * by + az * bz) / (
                math.sqrt(ax * ax + ay * ay + az * az) * math.sqrt(bx * bx + by * by + bz * bz)))
    sign = 180.0 / math.pi * math.acos((nbx * (x2 - x1) + nby * (y2 - y1) + nbz * (z2 - z1)) / (
                math.sqrt(nbx * nbx + nby * nby + nbz * nbz) * math.sqrt(
            ...
            for i in range(start, len(inlines)):
                if len(inlines[i].split()) == 0:
                    ...
                    self.CARTESIANS.append(
                        [float(inlines[i].split()[1]), float(inlines[i].split()[2]), float(inlines[i].split()[3])])
            ...
            if format == "Gaussian":
                for i in range(0, len(outlines)):
                    ...
                    if outlines[i].find("Input orientation") > -1:
                        standor2 = i
                        arb = 1
                    if outlines[i].find("Rotational constants") > -1 and outlines[i - 1].find(
                            "-------") > -1 and arb == 0:
                ...
                try:
                    ...
                    for i in range(standor + 5, standor + 5 + self.NATOMS):
                        ...
                        if len(outlines[i].split()) == 6:
                            self.CARTESIANS.append([float(outlines[i].split()[3]), float(outlines[i].split()[4]),
                                                    float(outlines[i].split()[5])])
                        else:
                            self.CARTESIANS.append([float(outlines[i].split()[2]), float(outlines[i].split()[3]),
...
class calcSterimol:
    def __init__(self, file, radii, atomA, atomB, verbose):
        ...
        if verbose == True:
            ...
            print("\n", "   Atom ".ljust(9), "  Xco/A".rjust(9), "  Yco/A".rjust(9), "  Zco/A".rjust(9),
                  " VdW/pm".rjust(9))
            print("   ##############################################")
        ...
        for x in range(len(twodcarts)):
            ...
            for y in range(len(twodcarts)):
                if x != y:
                    try:
                        nvect = (twod_vect(center, twodcarts[x],
                                           twodcarts[y]))  # origin normal vector to connecting atomic centers vector
                    except ValueError:
                        nvect = [0, 0]
                    iav = np.subtract(twodcarts[x], twodcarts[y])  # interatomic vector
                    iad = np.linalg.norm(iav)  # interatomic distance
                    try:
                        theta = math.asin((fragrad[y] - fragrad[
                            x]) / iad)  # calculates angle by which to rotate vdw radii before adding
                    except ValueError:
                        theta = np.pi / 2
                    try:
                    ...
                    nvect = (twod_vect(center, twodcarts[x] + xradv, twodcarts[
                        y] + yradv))  # origin normal vector to connecting atomic surfaces tangential vector
                    ...
                    if np.cross(nvect, xradv) < 0.000000001 and theta != np.pi / 2:
                        ...
                        if len(satpoint) == len(atomlist) - 2: vlist.append(np.linalg.norm(nvect));alist.append(
                            [x, y]);  # print x,y
                        satpoint = []
                        for z in range(len(twodcarts)):
...
def calcSandwich(file):
    ...
    for i in range(len(fileData.ATOMTYPES)):
        for j in range(len(fileData.ATOMTYPES)):
            dist = ((fileData.CARTESIANS[i][0] - fileData.CARTESIANS[j][0]) ** 2 + (
                        fileData.CARTESIANS[i][1] - fileData.CARTESIANS[j][1]) ** 2 + (
                                fileData.CARTESIANS[i][2] - fileData.CARTESIANS[j][2]) ** 2) ** 0.5
            ...
            for c in concheck(conpar, y):
                ...
                for d in concheck(conpar, x):
                    ...
                    for e in concheck(conpar, w):
                        ...
                        if z in concheck(conpar, v) and sorted(rar) not in allrings and len(
                            set(rar)) == 5: allrings.append(sorted(rar))
                        for f in concheck(conpar, v):
                            u = f
                            tar = []
                            tar.extend([z, y, x, w, v, u])
                            if z in concheck(conpar, u) and sorted(tar) not in allrings and len(
                                set(tar)) == 6: allrings.append(sorted(tar))

    if not allrings:
        for ma in metalatoms:
            for s in range(len(fileData.CARTESIANS)):
                if 0.1 < np.linalg.norm(
                    np.subtract(fileData.CARTESIANS[ma], fileData.CARTESIANS[s])) < 2.1: allrings.append(
                    [s, s, s, s, s])
    ...
    for ring in allrings:
        ...
        if len(ring) == 5:
            ...
            for i in frag:
                ...
                if ring[0] != ring[1]: omega.append(dihedral([10, 10, 10], getcoords(metal, fileData.CARTESIANS), cent,
                                                             getcoords(i, fileData.CARTESIANS)))
                if ring[0] != ring[1] and i in ring: ringang.append(
                    dihedral([10, 10, 10], getcoords(metal, fileData.CARTESIANS), cent,
                             getcoords(i, fileData.CARTESIANS)))
            ...
            for x in range(len(twodcarts)):
                for y in range(len(twodcarts)):
                    if x != y:
                        try:
                            nvect = (twod_vect(center, twodcarts[x], twodcarts[
                                y]))  # origin normal vector to connecting atomic centers vector
                        except ValueError:
                            nvect = [0, 0]
                        iav = np.subtract(twodcarts[x], twodcarts[y])  # interatomic vector
                        iad = np.linalg.norm(iav)  # interatomic distance
                        ...
                        if np.cross(nvect, xradv) < 0.000000001 and theta != np.pi / 2:
                            ...
                            if len(satpoint) == len(frag) - 2: vlist.append(np.linalg.norm(nvect));alist.append(
                                [x, y])  # ;print x,y
            B1 = round(min(vlist), 3)
            print("   " + file.ljust(25), str(tolmanCA).rjust(9), str(smcdist).rjust(9), str(lval).rjust(9),
                  str(B1).rjust(9), str(newB5).rjust(9))
