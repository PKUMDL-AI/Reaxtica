...
def get_qmdesc_SMC(x):
    h_sml, b_sml, ligand, reagent, solvents = x['h_mol'], x['b_mol'], x['ligand'], x['reagent'], Chem.MolFromSmiles(
        x['solvent'])
    ...
    solvent_list = Chem.MolToSmiles(solvents).split('.')
    solvent_name = [solvent_dict[s] for s in solvent_list]
...
def gen_SMC_input():
    df = pd.read_csv('datasets/USPTO_SMC.csv')
    dscp = df[['h_mol', 'b_mol', 'ligand', 'reagent', 'solvent']].apply(lambda x: get_qmdesc_SMC(x), axis=1)
    dscp = np.array(dscp)
    np.save(f'saved_descriptors/SMC_dscp_USPTO.npy', dscp)


def classifier_yield_RF(num_fold=5, model_name='USPTO.model', recalc=False):
    analyzed = pd.read_excel('../Utils/analyzed_descriptor.xlsx').SMC_R2_USPTO.values
    if not os.path.exists('saved_descriptors/SMC_dscp_USPTO.npy') or recalc:
        gen_SMC_input()
...
def predict_single_SMC(x, model_name='best.model'):
    try:
        analyzed = pd.read_excel('../Utils/analyzed_descriptor.xlsx').SMC_R2_USPTO.values
        SMC_subs = x.split(',')
        if len(SMC_subs) == 5:
            SMC_sml = {'h_mol': SMC_subs[0], 'b_mol': SMC_subs[1], 'ligand': SMC_subs[2], 'reagent': SMC_subs[3], 'solvent': SMC_subs[4]}
        else:
            SMC_subs = x.split(';')
            SMC_sml = {'h_mol': SMC_subs[0], 'b_mol': SMC_subs[1], 'ligand': SMC_subs[2], 'reagent': SMC_subs[3],
                       'solvent': SMC_subs[4]}
...
def predict_file_SMC(filepath, model_name='best.model', save_name='predict_output.csv'):
    analyzed = pd.read_excel('../Utils/analyzed_descriptor.xlsx').SMC_R2_USPTO.values
    df = pd.read_csv(f'datasets/{filepath}')
    dscp = df[['h_mol', 'b_mol', 'ligand', 'reagent', 'solvent']].apply(lambda x: get_qmdesc_SMC(x), axis=1)
    dscp = np.array([x.reshape(-1) for x in dscp])[:, [np.argsort(analyzed)[:37]]]
    model = joblib.load(f'saved_models/{model_name}')
...
if args.mode == 'retrain':
    classifier_yield_RF(args.times_retrain, args.model_name, args.recalc)
if args.mode == 'predict':
    predict_single_SMC(args.rxn_sml, args.model_name)
if args.mode == 'predict_file':
    predict_file_SMC(args.rxn_sml, args.model_name, args.save_name)
